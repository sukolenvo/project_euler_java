...
public class Problem082 {
  ...
  int run() throws Exception {
    @Cleanup InputStream content = getClass().getClassLoader().getResourceAsStream("problem082.txt");
    ...
    return findShortestPath(matrix,
        IntStream.range(0, matrix.length).mapToObj(x -> new Destination(x, 0, matrix[x][0])).collect(Collectors.toList()),
        destination -> destination.getY() == matrix.length - 1)
        .getDistance();
  }
